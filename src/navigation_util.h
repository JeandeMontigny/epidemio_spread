...
inline int GetBDMToMapLoc(double bdm_loc) {
  ...
  return nearbyint((bdm_loc + param->max_bound)/sparam->map_pixel_size);
}
// ---------------------------------------------------------------------------
inline double GetMapToBDMLoc(double map_loc) {
  ...
  return (map_loc * sparam->map_pixel_size) - param->max_bound;
}
...
inline double GetMapSize() {
  ...
  return (param->max_bound*2)/sparam->map_pixel_size;
}
