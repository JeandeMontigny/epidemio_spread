...
namespace bdm {
  ...
  inline TGeoManager* Buildcube() {
    ...
    // simulation volume - 12m * 3m * 3.5m
    TGeoVolume *sim_space = gGeoManager->MakeBox("sim_space", Air, 300, 50, 50);
    gGeoManager->SetTopVolume(sim_space);
    gGeoManager->SetTopVisible(0);
    ...
    // floor and roof
    mBlocks = geom->MakeBox("floor_roof", Iron, 150, 25, 1);
    mBlocks->SetLineColor(kBlack);
    ...
    // sides
    mBlocks = geom->MakeBox("left_right_sides", Iron, 150, 1, 25);
